...
private:
  ...
  void pack(RealView4D &halo_fn, Halos *send_buffers) {
    ...
    float64 *ptr_halo_fn = halo_fn.data();
    int     *ptr_map = send_buffers->map_.data();
    float64 *ptr_buf_flatten = send_buffers->buf_flatten_.data();
    auto idx_4d = halo_fn.index();
    auto idx_2d = send_buffers->map_.index();

    // Define accessors by macro
    #define _buf_flatten(j0) ptr_buf_flatten[j0]
    #define _map(j0, j1) ptr_map[idx_2d(j0, j1)]
    #define _halo_fn(j0, j1, j2, j3) ptr_halo_fn[idx_4d(j0, j1, j2, j3)]

    std::for_each_n(std::execution::par_unseq,
                    counting_iterator(0), total_size,
                    [=](const int idx){
  ...
  void unpack(RealView4D &halo_fn, Halos *recv_buffers) {
    ...
    float64 *ptr_halo_fn = halo_fn.data();
    int     *ptr_map = recv_buffers->map_.data();
    float64 *ptr_buf_flatten = recv_buffers->buf_flatten_.data();
    auto idx_4d = halo_fn.index();
    auto idx_2d = recv_buffers->map_.index();
  ...
  void local_copy(Halos *send_buffers, Halos *recv_buffers) {
    ...
    float64 *ptr_send_buf_flatten = send_buffers->buf_flatten_.data();
    float64 *ptr_recv_buf_flatten = recv_buffers->buf_flatten_.data();

    // Define accessors by macro
    #define _send_buf_flatten(j0) ptr_send_buf_flatten[j0]
    #define _recv_buf_flatten(j0) ptr_recv_buf_flatten[j0]
  ...
  void boundary_condition(RealView4D &halo_fn, Halos *send_buffers) {
    ...
    float64 *ptr_halo_fn = halo_fn.data();
    float64 *ptr_buf_flatten = send_buffers->buf_flatten_.data();
    int *ptr_map_bc  = send_buffers->map_bc_.data();
    int *ptr_map_orc = send_buffers->map_orc_.data();
    int *ptr_sign1   = send_buffers->sign1_.data();
    int *ptr_sign2   = send_buffers->sign2_.data();
    int *ptr_sign3   = send_buffers->sign3_.data();
    int *ptr_sign4   = send_buffers->sign4_.data();

    auto idx_4d      = halo_fn.index();
    auto idx_map_bc  = send_buffers->map_bc_.index();
    auto idx_map_orc = send_buffers->map_orc_.index();
    auto idx_sign1   = send_buffers->sign1_.index();
    auto idx_sign2   = send_buffers->sign2_.index();
    auto idx_sign3   = send_buffers->sign3_.index();
    auto idx_sign4   = send_buffers->sign4_.index();

    // Define accessors by macro
    #define _halo_fn(j0, j1, j2, j3) ptr_halo_fn[idx_4d(j0, j1, j2, j3)]
    #define _map_bc(j0, j1, j2) ptr_map_bc[idx_map_bc(j0, j1, j2)]
    #define _map_orc(j0, j1) ptr_map_orc[idx_map_orc(j0, j1)]
    #define _sign1(j0, j1, j2) ptr_sign1[idx_sign1(j0, j1, j2)]
    #define _sign2(j0, j1, j2) ptr_sign2[idx_sign2(j0, j1, j2)]
    #define _sign3(j0, j1, j2) ptr_sign3[idx_sign3(j0, j1, j2)]
    #define _sign4(j0, j1, j2) ptr_sign4[idx_sign4(j0, j1, j2)]
    #define _buf_flatten(j0)   ptr_buf_flatten[j0]

    if(total_size0 > 0) {
      std::for_each_n(std::execution::par_unseq,
                      counting_iterator(0), total_size0,
                      [=](const int idx) {
    ...
    if(total_size1 > 0) {
      std::for_each_n(std::execution::par_unseq,
                      counting_iterator(0), total_size1,
                      [=](const int idx) {
                        const int idx_dst = _map_orc(idx, orcsum);
                        const int rx  = _map_bc(idx, orcsum, 0), ry  = _map_bc(idx, orcsum, 1);
                        const int rvx = _map_bc(idx, orcsum, 2), rvy = _map_bc(idx, orcsum, 3);
    ...
    if(total_size2 > 0) {
      std::for_each_n(std::execution::par_unseq,
                      counting_iterator(0), total_size2,
                      [=](const int idx) {
                        const int idx_dst = _map_orc(idx, orcsum);
                        const int rx  = _map_bc(idx, orcsum, 0), ry  = _map_bc(idx, orcsum, 1);
                        const int rvx = _map_bc(idx, orcsum, 2), rvy = _map_bc(idx, orcsum, 3);
    ...
    if(total_size3 > 0) {
      std::for_each_n(std::execution::par_unseq,
                      counting_iterator(0), total_size3,
                      [=](const int idx) {
                        const int idx_dst = _map_orc(idx, orcsum);
                        const int rx  = _map_bc(idx, orcsum, 0), ry  = _map_bc(idx, orcsum, 1);
                        const int rvx = _map_bc(idx, orcsum, 2), rvy = _map_bc(idx, orcsum, 3);
