...
namespace Advection {
  ...
  void advect_4D(Config *conf, Efield *ef, RealView4D &fn, RealView4D &tmp_fn, float64 dt) {
    ...
    float64 *ptr_fn = fn.data();
    float64 *ptr_tmp_fn = tmp_fn.data();
    float64 *ptr_ex = ef->ex_.data();
    float64 *ptr_ey = ef->ey_.data();
    auto idx_4d = fn.index();
    auto idx_2d = ef->ex_.index();

    // Define accessors by macro
    #define _fn(j0, j1, j2, j3) ptr_fn[idx_4d(j0, j1, j2, j3)]
    #define _tmp_fn(j0, j1, j2, j3) ptr_tmp_fn[idx_4d(j0, j1, j2, j3)]
    #define _ex(j0, j1) ptr_ex[idx_2d(j0, j1)]
    #define _ey(j0, j1) ptr_ey[idx_2d(j0, j1)]
