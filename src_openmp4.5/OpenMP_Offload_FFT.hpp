#ifndef __OPENMP_OFFLOAD_FFT_HPP__
#define __OPENMP_OFFLOAD_FFT_HPP__

/*
 * Simple wrapper for FFT class (only 2D FFT implemented now)
 * CUFFT interface for OpenMP
...
namespace Impl {
  ...
  struct FFT {
    ...
    void rfft2(RealType *dptr_in, Complex<RealType> *dptr_out) {
      // It seems that, this cannot detect device pointer on non-zero GPU
